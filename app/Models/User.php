...
class User extends Authenticatable
{
    ...
    {
        return $this->belongsToMany(MasterCustomer::class, 'user_customers', 'user_id', 'master_customer_id');
    }

    public function hasPermission($permissionName): bool
    {
        // Cache or retrieve role permissions for the specific role
        $rolePerm = \App\Models\RolePermission::where('role', $this->role)->first();
        if (!$rolePerm) {
            return false;
        }

        $perms = $rolePerm->permissions ?? [];

        // If the role has 'Full Access', they can do anything
        if (in_array('Full Access', $perms)) {
            return true;
        }
