...
class PurchaseRequestController extends Controller
{
    public function index()
    {
        ...
        $prs = DB::table('purchase_requests as pr')
            ...
            ->select(
                ...
                DB::raw('MAX(bp.department) as department_code'),
    ...
    public function create()
    {
        ...
        $ios = DB::table('budget_plans')
            ...
            ->selectRaw('(SELECT COALESCE(SUM(bi.total_amount), 0) FROM budget_items bi WHERE bi.plan_id = budget_plans.id) as total_budget')
    ...
    public function store(Request $request)
    {
        ...
        try {
            foreach ($request->items as $item) {
                // $item structure: master_item_code, budget_item_id, description, uom, qty, estimated_price
                
                $pr = new PurchaseRequest();
                $pr->pr_number = $pr_number;
                $pr->budget_item_id = !empty($item['budget_item_id']) ? $item['budget_item_id'] : 0; 
                ...
                // Asset No (AUC), G/L Account, Storage Location, Due Date
                $pr->asset_no = $request->asset_no ?? null;
                $pr->gl_account = $request->gl_account ?? null;
    ...
    public function show($id)
    {
        ...
        // Extract Header Info from first item's notes
        $firstNote = $items->first()->notes;
        $header = [
            'department' => '-',
            'io_number' => '-',
            'plant' => '-',
        ...
        $parts = explode(' | ', $firstNote);
        foreach($parts as $part) {
            if(str_starts_with($part, 'Dept: ')) $header['department'] = substr($part, 6);
            if(str_starts_with($part, 'IO: ')) $header['io_number'] = substr($part, 4);
            if(str_starts_with($part, 'Plant: ')) $header['plant'] = substr($part, 7);
            if(str_starts_with($part, 'Cat: ')) $header['category'] = substr($part, 5);
            if(str_starts_with($part, 'CC: ')) $header['cost_center'] = substr($part, 4);
        }
        
        // Add fields from PR record
        $firstItem = $items->first();
        $header['asset_no'] = $firstItem->asset_no ?? '';
        $header['gl_account'] = $firstItem->gl_account ?? '';
        $header['storage_location'] = $firstItem->storage_location ?? '';
        $header['due_date'] = $firstItem->due_date ?? '';
            ...
            foreach ($request->items as $itemId => $data) {
                ...
                if(str_starts_with($itemId, 'new_')) {
                    ...
                    if($prItem) {
                        ...
                        // Update new fields
                        $prItem->asset_no = $request->asset_no;
                        $prItem->gl_account = $request->gl_account;
    ...
    public function destroy($id)
    {
        ...
        if ($user->role !== 'Admin' && $user->role !== 'Super Admin') {
            return back()->with('error', 'Unauthorized. Only Admin can delete PRs.');
        }
